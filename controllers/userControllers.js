...
exports.getUserDashboard = async (req, res) => {
  ...
  let total_resolved = result.filter(
    (item) => item.status === "resolved"
  ).length;
  let total_pickup_request = result.filter(
    (item) => item.request_type === "Pickup"
  ).length;
  let total_complaint_request = result.filter(
    (item) => item.request_type === "Complaint"
  ).length;
  let total_recycling_request = result.filter(
    (item) => item.request_type === "Recycling"
  ).length;
  let total_other_request = result.filter(
    (item) => item.request_type === "Other"
  ).length;
  let total_unassigned_driver_requests = result.filter(
    (item) => item.assignedDriver === "none"
  ).length;
...
exports.getRaiseRequestPage = async (req, res) => {
  ...
  try {
    ...
    res.render("user/request.ejs", { result: username });
  } catch (err) {
...
exports.submitRequest = async (req, res) => {
  try {
    ...
    const requestDetails = `
      Email: ${data_to_insert_in_db.email}
      Request Type: ${data_to_insert_in_db.request_type}
      Address: ${data_to_insert_in_db.address}
      Message: ${data_to_insert_in_db.message}
      Time: ${data_to_insert_in_db.time}
      Status: ${data_to_insert_in_db.status}
      Assigned Driver: ${data_to_insert_in_db.assignedDriver}
    `;

    const emailSubject = "New Request Submitted";
    const emailBody = `
      New Request Submitted

      A new request has been submitted by ${email}

      Request Details:
      ${requestDetails}

      Login to admin dashboard: http://localhost:3000/admin/all-requests
    `;
    ...
    res.send(
      "Your request has been submitted to our database and an email notification has been sent to the admin."
    );
  } catch (err) {
...
exports.getMyRequests = async (req, res) => {
  ...
  try {
    ...
    const user = await db.collection("users").findOne({ email });
    const username = user.name;
    res.render("user/my-requests.ejs", {
